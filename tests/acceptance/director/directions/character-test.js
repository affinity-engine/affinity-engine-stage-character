...
test('Ember Theater | Director | Directions | character', function(assert) {
  ...
  visit('/ember-theater/test-scenarios/director/directions/character').then(() => {
    ...
    assert.ok(Ember.$(`${hook('expression_direction')} img`).attr('src').match('theater/characters/bitsy/neutral.png'), 'it sets the `src` based on the associated fixture expression');
    ...
    assert.ok(Ember.$(`${hook('expression_direction')}:nth(3) img`).attr('src').match('theater/characters/emma/neutral.png'), 'the manually defined character defaultExpressionId is set properly');
    ...
    assert.ok(Ember.$(`${hook('expression_direction')}:nth(5) img`).attr('src').match('theater/characters/bitsy/happy.png'), '`expression` can adjust the expression before rendering');

    return step(100);
  }).then(() => {
    assert.ok(Ember.$(`${hook('expression_direction')}:nth(5) img`).attr('src').match('theater/characters/bitsy/sad.png'), '`expression` can adjust the expression after rendering');
